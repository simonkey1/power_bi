...
Una vez realizado estos pasos, nos aparecerá una ventana emergente, donde presionaremos en Transform Data. A través de esta hoja de cálculo podremos completar los requerimientos para poder visualizar nuestro mapa de Chile por regiones. Estos son:

- **Ordenar de mayor a menor los cortes de luz**
- **Construir una nueva columna llamada Region_Corregida a partir de nuestra variable región**
- **Renombrar cada región de esta columna con los nombres del archivo topojson(mapa)**
...
## **Nueva columna región corregida** 

En el menú principal buscamos la sección Add column, y clickeamos en Custom column, Luego ponemos el siguiente código:


```M
= Table.AddColumn(#"Sorted Rows", "Region_Corregida", each "Región de " & [REGION])

``` 

📌 **¿Qué hace esto?**

 - **Table.AddColumn → Función que agrega una nueva columna a una tabla existente.**
 - **#"Sorted Rows" → Nombre de nuestra base ya ordenada (paso anterior) en la que se está agregando la nueva columna.**
 - **"Region_Corregida" → Nombre de la nueva columna que se está creando.**
 - **each "Región de " & [REGION] → Expresión que se evalúa para cada fila:**
 - **"Región de " → Texto fijo.**
 - **[REGION] → Columna existente en la tabla, cuyos valores se concatenan con el texto fijo.**
